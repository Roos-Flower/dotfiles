...
def main(): Unit =
  ...
  Using(stackPush())(stack =>
    ...
    val vertexAmt=vertexVboSeq.length/3

    val vertexVbo = createVbo(vertexVboSeq*)
    
    val colorVbo = createVbo(
      1, 0, 0,
      0, 1, 0,
      0, 0, 1,
    )
    ...
    // we can just bind the VAO and instantly render the mesh
    // with no hassle! Very nice indeed! :)
    val vao = createVao(vertexVbo, colorVbo)
    ...
    while !glfwWindowShouldClose(window) do
      ...
      glDrawArrays(GL_TRIANGLE_STRIP, 0, vertexAmt)
